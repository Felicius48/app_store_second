...
npm run init-db
if [ $? -ne 0 ]; then
    echo "⚠️  Ошибка инициализации базы данных. Пробуем пересобрать sqlite3..."
    npm rebuild sqlite3 --build-from-source
    if [ $? -ne 0 ]; then
        echo "❌ Ошибка пересборки sqlite3"
        exit 1
    fi
    npm run init-db
    if [ $? -ne 0 ]; then
        echo "❌ Ошибка инициализации базы данных после пересборки sqlite3"
        exit 1
    fi
fi
